...
async function addToGenre(data) {
  ...
  const trackDataFromPlayer = await trackPlayerService.getTrackPropertiesFromPlayer(trackToCreate)
...
async function verifyIfCanAddTrackToGenre(trackToCreate, musicGenreId) {
  ...
  await trackManager.verifyIfTrackDoesNotAlreadyExistsInGenre(trackToCreate, musicGenre)
  return musicGenre
}
...
async function checkIfUserHasUpvotedTheTracks(tracks, userHash) {
  return bluebird.map(tracks, async (track) => {
    const hasUpvoted = await trackManager.hasUserUpvotedTheTrack(track.id, userHash)
    return {
      ...track,
      hasUpvoted,
    }
